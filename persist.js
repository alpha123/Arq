...
var persist = (function () {
    ...
    function fileExists(path)
    {
        ...
        var files = GetFileList(pathBits.dir.charAt(0) == '~' ? pathBits.dir : 'other/' + pathBits.dir);
	...
	var visited = [];
	function doTie(obj, proto)
	{
            var protoTie = function () {};
            protoTie.prototype = proto;
            var newObj = new protoTie();
            for (var p in obj) {
		if (p in proto && typeof proto[p] == 'object' && !visited.contains(obj[p])) {
		    visited.push(obj[p]);
                    newObj[p] = doTie(obj[p], proto[p]);
		}
		else
                    newObj[p] = obj[p];
            }
    ...
    {
        return hasOwn.call(getWorldState(), map);
    }
    ...
    {
        return hasOwn.call(object, member) && Object.prototype.propertyIsEnumerable.call(object, member);
    }
	...
	for (var persons = GetPersonList(), p = 0; p < persons.length; ++p) {
            if (member == persons[p])
                return false;
        }
    ...
    function initMapState(map, script)
    {
        ...
        for (var element in script) {
            if (hasOwn.call(script, element) && script[element] != 'function' && isCustomMember(script, element) && isMapVariable(element))
    ...
    function initPersonState(map, person, script)
    {
        var personState = {};
        for (var element in script) {
	    if (hasOwn.call(script, element) && typeof script[element] != 'function' && isCustomMember(script, element) && isPersonVariable(element))
    ...
    /* List of map script processor functions. */
    var mapScriptProcessors = [];

    /* Add a new map script processor. */
    function addMapScriptProcessor(processor) {
	mapScriptProcessors.push(processor);
    }

    /* Remove all occurrences of a processor from the list. */
    function removeMapScriptProcessor(processor) {
	for (var p = 0; p < mapScriptProcessors.length; ++p) {
	    if (mapScriptProcessors[p] === processor)
		mapScriptProcessors.splice(p, 1);
	}
    }

    /* Run any map script processors. */
    function runMapScriptProcessors(mapScript, map)
    {
	logNote("runMapScriptProcessors: Running " + mapScriptProcessors.length + " script processors on script for " + map);
	for (var p = 0; p < mapScriptProcessors.length; ++p)
	    mapScript = mapScriptProcessors[p](mapScript, map);
	return mapScript;
    }
    ...
    {
        if (hasOwn.call(scriptCache, map))
            return scriptCache[map];

        try {
            scriptCache[map] = eval(runMapScriptProcessors(loadMapScriptFile(map), map));
        } catch (e) {
    ...
    /* Re-run the create script of a person. */
    function recreatePerson(person)
    {
	logNote("recreatePerson: Re-running create script for " + person);
	CallPersonScript(person, SCRIPT_ON_CREATE);
    }
    ...
    /* Connect events of a person to scripts. */
    function bindPersonEvents(person)
    {
	var mapScript = getMapState();
	for (var e = 0; e < personEvents.length; ++e) {
            if (person in mapScript && personEvents[e].fn in mapScript[person]) {
		logNote("bindPersonEvents: Binding " + personEvents[e].fn + " for " + person);
                SetPersonScript(person, personEvents[e].event, 'persist.triggerPersonEvent("' + personEvents[e].fn + '");');
            }
        }
    }
    ...
    function bindPersonsEvents()
    {
        ...
        var persons = getImportantPersons();
        for (var p = 0; p < persons.length; ++p)
            bindPersonEvents(persons[p]);
    }

    /* Disconnect events of a person from scripts. */
    function unbindPersonEvents(person)
    {
	for (var e = 0; e < personEvents.length; ++e) {
            logNote("unbindPersonEvents: Unbinding " + personEvents[e].fn + " from " + person);
            SetPersonScript(person, personEvents[e].event, '');
        }
    }
	...
	addScriptProcessor: addMapScriptProcessor,
	removeScriptProcessor: removeMapScriptProcessor,
        init: init,
        stop: stop,
	...
	recreatePerson: recreatePerson,
        recreatePersons: recreatePersons,
	bindPersonEvents: bindPersonEvents,
        bindPersonsEvents: bindPersonsEvents,
	unbindPersonEvents: unbindPersonEvents,
	unbindPersonsEvents: unbindPersonsEvents
    };
})();
